...
fig1.update_layout(
    ...
    title=dict(
                text="Linear Fit of Reaction Enthalpies_correlation-consistent",
...
# Save as PNG
fig1.write_image("fit_cc_enthalpy.png", format="png", scale=5)

# save a html
import plotly.io as pio
pio.write_html(fig1, 'fit_cc_enthalpy.html', auto_open=False, )
...
for i, (method, gibbs) in enumerate(gibbs_data.items()):
    ...
    fig2.add_trace(go.Scatter(x=x_valid, y=y_valid, mode="markers", name=f"{method} Data",
                             marker=dict(color=colors[i])),  
                  row=row, col=col)   
    fig2.add_trace(go.Scatter(x=x_fit, y=y_fit, mode="lines", name=f"{method} Fit: y = {result.params['slope'].value:.2f}x + {result.params['intercept'].value:.2f}, R² = {r_squared:.2f}",
                             line=dict(color=colors[i])),
                  row=row, col=col)
...
fig2.update_layout(
    width=1000, height=700,
    title=dict(
                text="Linear Fit of Change in Gibbs Energy_correlation-consistent",
                font=dict(size=22, family="Arial", color="black"),
                x=0.5, y=0.97,  # Center the title
                ),
    font=font_style,
...
# Save as PNG
fig2.write_image("fit_cc_gibbs.png", format="png", scale=5)

# save a html
import plotly.io as pio
pio.write_html(fig2, 'fit_cc_gibbs.html', auto_open=False, )
...
fig3.update_layout(
    width=1000, height=700,
    title=dict(
                text="Linear Fit of Reaction Entropies_correlation-consistent",
                font=dict(size=22, family="Arial", color="black"),
                x=0.5, y=0.97,  # Center the title
                ),
...
# Save as PNG
fig3.write_image("fit_cc_entropy.png", format="png", scale=5)

# save a html
import plotly.io as pio
pio.write_html(fig3, 'fit_cc_entropy.html', auto_open=False, )
